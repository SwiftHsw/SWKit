...
/// 快捷创建属性
#define SWCreateProperty(propertyModifier, propertyPointerType, propertyName)     \
@property(nonatomic,propertyModifier)propertyPointerType  propertyName;  \


///暗黑模式颜色
#define SWDarkColor(light,dark)    \
({\
    UIColor *swMenuIndicator = nil; \
    if (@available(iOS 13.0, *)) {  \
        swMenuIndicator = [UIColor colorWithDynamicProvider:^UIColor * _Nonnull(UITraitCollection * _Nonnull traitCollection) { \
...
///> iOS 11 的判断iphoneX系列
#define ios11AIphoneX ({\
BOOL isPhoneX = NO;\
if (@available(iOS 11.0, *)) {\
if (SWWindow.safeAreaInsets.bottom > 0.0) {\
isPhoneX = YES;\
}\
}\
isPhoneX;\
