...
#import "AppNetworkTool.h"
#import <AFNetworking/AFNetworking.h>
 
@implementation AppNetworkTool

+ (instancetype)shareInstance{
    static AppNetworkTool *netTool = nil ;
 ...
 *   监听网络状态的变化
 */
+ (void)checkingNetworkResult:(void (^)(SWNetworkStatus))result {
    ...
    [reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        if (status == AFNetworkReachabilityStatusUnknown) {
            if (result) result(SWNetworkStatusUnknown);
        }else if (status == AFNetworkReachabilityStatusNotReachable){
            if (result) result(SWNetworkStatusNotReachable);
        }else if (status == AFNetworkReachabilityStatusReachableViaWWAN){
            if (result) result(SWNetworkStatusReachableViaWWAN);
        }else if (status == AFNetworkReachabilityStatusReachableViaWiFi){
            if (result) result(SWNetworkStatusReachableViaWiFi);
        }
    }];
}
